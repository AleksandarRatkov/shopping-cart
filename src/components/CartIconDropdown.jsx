...
const CartIconDropdown = ({pathname}) => {

    const cartItemsNumber = useSelector(selectAllCartItemsCount)
    const cartItems = useSelector(selectAllCartItems)
    const dispatch = useDispatch()
    const [runCartAnimation, setRunCartAnimation] = useState(false)
    ...
    useEffect(() => {
        if (cartItemsNumber !== 0) {
            setRunCartAnimation(true)
            setTimeout(() => {
                setRunCartAnimation(false)
            }, 500);
        }
    }, [cartItemsNumber]);
    ...
    return (
        <li className={`nav-item dropdown mr-50 ${pathname === "/cart" && 'hidden'}`} id="navigation-dropdowm">
            <button className={`btn btn-primary-outline borderless ${runCartAnimation && 'animate__animated animate__heartBeat animate__slow'}`} type="button" data-bs-toggle="dropdown" aria-expanded="false">
            ...
            <span className="position-absolute translate-middle badge rounded-pill badge-bg">
                <span className="animate__animated animate__bounce">{cartItemsNumber}</span>
